...
def api_interpret():
    """
    Minimal stub for now (so frontend can be wired).
    Later you’ll replace with real LLM call and strict JSON parsing.
    """
    payload = request.get_json(force=True) or {}
    query = payload.get("query", "")

    # simple rule-based fallback (temporary)
    intent_label = "ROUTE_FASTEST"
    constraints = {"avoidStairs": False, "requireElevator": False, "avoidHazards": False}
    weights = {"time": 1.0, "crowd": 0.25, "risk": 0.25, "distance": 0.1}

    q = str(query).lower()
    if "wheelchair" in q or "no stairs" in q or "accessible" in q:
        intent_label = "ROUTE_ADA"
        constraints = {"avoidStairs": True, "requireElevator": True, "avoidHazards": False}
        weights = {"access": 1.0, "risk": 0.3, "time": 0.2, "crowd": 0.2, "distance": 0.1}
    elif "least crowded" in q or "quieter" in q or "avoid crowd" in q:
        intent_label = "ROUTE_LEAST_CROWDED"
        constraints = {"avoidStairs": False, "requireElevator": False, "avoidHazards": False}
        weights = {"crowd": 1.0, "time": 0.35, "risk": 0.25, "distance": 0.15}
    elif "emergency" in q or "smoke" in q or "fire" in q:
        intent_label = "ROUTE_EMERGENCY_SAFEST"
        constraints = {"avoidStairs": False, "requireElevator": False, "avoidHazards": True}
        weights = {"risk": 1.0, "crowd": 0.5, "time": 0.35, "access": 0.1}
